...
export function PostDetail({ post }) {
    ...
    return (
        <>
            ...
            {deleteMutation.isLoading && <p style={{ color: 'blue' }}>Deleting post...</p>}
            {deleteMutation.isSuccess && <p style={{ color: 'green' }}>Post has (not) been deleted</p>}
            <button onClick={() => updateMutation.mutate(post.id)}>Update title</button>
            {updateMutation.isError && <p style={{ color: 'red' }}>Error update post</p>}
            {updateMutation.isLoading && <p style={{ color: 'blue' }}>Updating post...</p>}
