...
export default class Particle {
    ...
    draw(ctx) {
        ctx.translate(this.x + this.width * 1.2, this.y + this.height * 1.2); // 회전 중심축을 0.0에서 사각형의 중심으로 이동
        ctx.rotate((Math.PI / 180) * this.rotation); // 1라디안만큼 회전
        ctx.translate(-this.x - this.width * 1.2, -this.y - this.height * 1.2); // 회전 중심축을 다시 0.0으로 초기화
